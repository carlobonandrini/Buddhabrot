...
int main()
{
	...
	sf::RenderWindow window(sf::VideoMode(winSize, winSize), "Buddhabrot Visualization");
	...
	std::mt19937 rng;
	std::uniform_real_distribution<double> realDistribution(minimum.r(), maximum.r());
	std::uniform_real_distribution<double> imagDistribution(minimum.i(), maximum.i());
		...
		if (changedSomething) {
			...
			for (int k = 0; k < maxPoints; ++k)
			{
				...
				for (Complex n : pointPassed) {
					if (n.i() <= maximum.i() && n.i() >= minimum.i() && n.r() <= maximum.r() && n.r() >= minimum.r()) {
